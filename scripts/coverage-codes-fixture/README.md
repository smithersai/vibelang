...
Codes are in the `VIBE9xxx` range, which no implementation uses, so a
fixture leaking into a real derivation would be obvious rather than plausible.

- `VIBE90xx` — REPORT sites. Each must be counted.
- `VIBE91xx` — MENTIONS only. Each must be rejected.
...
**This file is one of the specimens.** `VIBE9104` appears in the sentence you
are reading, exactly as `VIBE1708` appeared in `compiler/FORK-SEAM-DESIGN.md`
...
otherwise let through:
    ...
    the rule this replaced used to read report(node, "VIBE9104", "gone")
