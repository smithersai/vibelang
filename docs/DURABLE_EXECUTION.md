...
- **`failures.mdx` §Refusal Conditions is ahead of the implementation — `(SA-1)`.** "Placement is unrestricted" and the six-form accepted list are not supported today. Re-measured 2026-08-27 against the conformance reference backend over `poc/src`: **two** of the six compile — `r!.length` and `r! ?? "fallback"`, each with its own certifying corpus case — and `r!.trim()`, `r![0]`, `maybe ?? r!` and `f(r!)` are refused with `SMITHERS1204`, the withdrawn statement-walk rule. (An earlier pass reported only `r!.length` as compiling; the `r! ?? "fallback"` row was a must-use artifact of that pass's scaffold, not a refusal. The corpus case `02-unwrap-propagation/postfix-bang-as-a-nullish-left-operand-is-accepted` is the direct evidence.) Five corpus cases in `02-unwrap-propagation` certify the refusals. Recorded, not resolved; recorded as open question 6 in `DECISIONS.md`.
- **The durable frontend still implements the plan model — `(SA-2)`.** A runtime branch inside a `durable(...)` body is refused with `SMITHERS4106` and a runtime loop with `SMITHERS4107`, whose own message reads "require parameterized Plan templates". `Action.run` is typed `A | undefined` rather than `Result<A, E>`, so the annotated Flow signature that this file and `durable-execution.mdx` both show does not check — measured as `SMITHERS4100` — and a plain `return` inside a durable body is not Result-lifted. The corpus certifies the refusals, so no `product-divergence.json` row records any of this.
