---
title: VibeLang · A new programming language for Agents
layout: minimal
description: A TypeScript superset with the language-level primitives for building reliable tools and agents.
showAskAi: false
showLogo: true
showOutline: false
showSidebar: false
...
<div className="vibe-home">
  ...
  <section className="vibe-home__hero">
    <div className="vibe-home__intro">
      <div className="vibe-home__eyebrow">A new programming language for Agents</div>
      <h1><span className="vibe-gradient">Vibe</span>Lang</h1>
      <div className="vibe-home__tagline">
        A TypeScript superset with the <span>language-level primitives</span> for building <span>reliable tools and agents</span>.
      </div>
      <div className="vibe-home__actions">
        ...
        <HomePage.Button href="/introduction/why-vibelang">Why VibeLang?</HomePage.Button>
        <HomePage.Button href="https://github.com/smithersai/vibelang">GitHub</HomePage.Button>
      </div>
    </div>

    <div className="vibe-home__try">
  ...
  :::
      ...
      <div className="vibe-home__stats">
        <div className="vibe-stat">
          <span>stage</span>
          <strong>prealpha</strong>
        </div>
        <div className="vibe-stat">
          <span>license</span>
          <strong>MIT</strong>
  ...
  <section className="vibe-home__cards">
    <article className="vibe-home__card">
      <h2>TypeScript compatible</h2>
      <div>Import TypeScript packages, use both languages side by side, and compile VibeLang to TypeScript.</div>
    </article>
    <article className="vibe-home__card">
      <h2>Native when ready</h2>
      <div>Optionally compile to native code—the easiest path from a TypeScript codebase to native performance.</div>
    </article>
    <article className="vibe-home__card">
      <h2>Comptime</h2>
    ...
    <article className="vibe-home__card">
      ...
      <div>Compile selected Actions and Flows into typed, resumable execution plans.</div>
    </article>
  </section>
</div>
...
const user = getUser(users, id) catch (error) {
...
The compiler sees `throw new NotFound(id)` and infers both the `User` success type and the `NotFound` failure type from the function body. The caller must propagate or handle that failure. Inside `catch`, the error is a typed value, so the `switch` must cover the complete failure set.

There is no wrapper value, generator ceremony, or universal effect interpreter. These are still eager functions with ordinary stacks.
...
abstract class AuditLog extends Context {
  abstract write(event: AuditEvent): void throws AuditError
}
...
function issueToken(user: User) {
  ...
  try audit.write({ type: "token.issued", userId: user.id })
  return token
}
...
```ts [VibeLang]
import config from "./agent.config.json" as const
...
import { rank } from "./rank.rs"
import { tokenize } from "./tokenize.zig"
...
### Concurrency stays structured

Async functions remain ordinary async functions. A structured join owns its child work and combines success values and typed failure sets without introducing a fiber runtime.
...
TypeScript's `Promise.all` rejects early while sibling work may continue. VibeLang's join cancels remaining children on failure and waits for their cleanup, so work cannot silently outlive the code responsible for it. Effect provides the same structured guarantee through Effect values and its fiber runtime.
...
abstract class Research extends Action<
  (input: ResearchInput) => Report throws ResearchFailed
> {}

abstract class Publish extends Action<
  (input: PublishInput) => Publication throws PublishFailed
> {}
...
The compiler derives schemas, failure channels, plan edges, and worker contracts. Completed Actions can be replayed after interruption without rerunning work that already succeeded. Effect adds typed composition and retry policies, but an Effect program is not durably journaled across process restarts by itself.
...
- **Typed failures** — use the same inferred, exhaustive failure model across synchronous and asynchronous functions
...
- **Structured concurrency** — use typed workers, structured joins, and cancellation without a fiber runtime
