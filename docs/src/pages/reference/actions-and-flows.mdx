...
A deployment is a canonical signed envelope over the exact Plan, manifest,
coordinator, worker pools, implementation and policy digests, schemas,
capability grants, and artifact identities. The coordinator verifies the
envelope against out-of-band trust roots before it constructs any worker
transport. A worker pool may use only a transport authorized for its exact
declared sandbox and placement.

The signature authenticates artifact provenance. It does not attest a live
