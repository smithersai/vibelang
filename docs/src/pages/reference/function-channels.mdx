...
The current POC appends a versioned `@vibeEffects` JSON tag to exported
`.d.mts` declarations. It records sorted `failures` and `requirements` and can
be decoded with `readDeclarationEffects` from `vibelang/language`. This is an
