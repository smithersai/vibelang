...
VibeLang owns the initial expression-form grammar while leaving room to converge with future TC39 pattern matching.
...
```ts
const port = config.port ?? throw InvalidConfig({ field: "port" })
```
...
function write(path: string, contents: string): !void {
  const file = try FileSystem.open(path, "write")
  defer file.close()
