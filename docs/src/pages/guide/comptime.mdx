...
`embed` is a second named import from the same module, not an ambient global.
Importing only `comptime` and calling `embed(...)` is refused with `VCT1004`,
"call is not a compiler intrinsic".
...
const Api = comptime(deriveApi(JSON.parse(embed("./api.json"))))
```

Compose inside **one** `comptime(...)`. Reading an existing comptime binding
from inside a second `comptime(...)` — `const a = comptime(1); const b =
comptime(a + 1)` — is refused with `VCT1004` on the producing call.
...
:::warning The documented target names are not reachable yet — measured 2026-08-28
The shipped `smithers` CLI hard-codes the target to `"node-es2022"` and no flag
changes it, so the lookup below fails with `VCT1004`, `property "node-es2022" is
absent from the comptime value`. The pattern is right; give the record a
`"node-es2022"` key, or key it on whatever `target` your own programmatic host
passes. See [Comptime](/reference/comptime) §Target Selection.
:::
