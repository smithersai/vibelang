...
1. A valid TypeScript program MUST remain valid VibeLang input for the TypeScript target.
2. Its observable TypeScript behavior MUST be preserved unless the source opts into VibeLang-specific syntax or APIs.
3. VibeLang MUST NOT impose a globally sound type system that invalidates TypeScript escape hatches.
4. Precision MAY be adopted incrementally across files and API boundaries.

VibeLang source uses the `.vibe` extension. The JSX-capable extension is **open**; `.vibex` is a candidate only.

## TypeScript Target

The TypeScript target MUST support the complete TypeScript language. VibeLang constructs SHOULD lower into ordinary TypeScript before the stock pipeline finishes checking and emission where feasible.

Failure and requirement annotations MUST erase where they do not require runtime machinery. The backend MUST NOT wrap every function result merely to encode its static channels.
...
Platform-specific globals such as `process`, `window`, `document`, filesystem, and network MUST NOT be unconditional globals in authored `.vibe` code. Host functionality MUST be represented by typed requirements.
