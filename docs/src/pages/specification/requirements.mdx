...
Resource acquisition and finalization MUST remain explicit ordinary code, initially through `using` or `defer`. A base Layer receives an already acquired service and MUST NOT own that service's lifetime. A future resource-building convenience MAY carry construction failures and requirements without changing the base Layer rule.
...
A base Layer MUST NOT discover, register, cancel, or join child work. Fire-and-forget and detached work are initially unavailable in authored `.vibe`; every started Promise MUST be consumed directly by `await` or by a recognized combinator whose resulting Promise is consumed before scope exit.
