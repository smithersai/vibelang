...
Using a runtime value from a TypeScript or JavaScript module adds the `TypeScript` requirement. Every unannotated foreign call MUST also add the distinguished checked `panic` case. Trusted `@throws {never}` metadata removes that default; `@throws {T}` or an adapter may supply a more precise channel.
