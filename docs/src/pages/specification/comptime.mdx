...
For supported data operations, evaluation and replacement MUST preserve
observable JavaScript property order, including `Object.keys`, `Object.values`,
`Object.entries`, and `JSON.stringify`. Canonical artifact encoding is not the
semantics of the program's `JSON.stringify`. A cache hit MUST preserve the same
property order as a fresh evaluation, and cache identity MUST distinguish values
whose observable order differs.

Within a supported data value, materialization MUST preserve shared object and
array references, and MUST NOT merge distinct allocations merely because their
contents are equal. Argument/result transport and cache reloads MUST preserve
that sharing too. Cache identity MUST distinguish alias layouts that change
observable reference equality; a plain JSON tree is not a sufficient encoding
for such values.
