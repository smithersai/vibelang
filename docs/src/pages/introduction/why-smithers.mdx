...
Durable runtimes commonly repeat schemas, error shapes, dependencies, and RPC bindings that the compiler already knows. Passing a statically resolvable function to `durable(...)` lets Smithers lower checked code into Plan IR and derive those boundaries. Planning reads emitted IR; it never executes the Flow or its Actions.
