---
title: Overview
description: An overview of VibeLang's language model, features, and design goals.
---
...
VibeLang is not a syntactic superset of TypeScript. Shared syntax keeps TypeScript behavior unless a divergence is explicitly documented, while imported `.ts`, `.tsx`, and JavaScript modules retain their own complete syntax and runtime semantics. On the TypeScript target, `.vibe` constructs lower to ordinary TypeScript/JavaScript where feasible.
...
VibeLang is *designed* to import local Rust and Zig sources, drive the foreign
...
the two foreign imports below are refused with `VIBE5213` today. The `json`,
...
Use the TypeScript ecosystem as it is — its packages, tools, and deployment infrastructure. VibeLang emits ordinary TypeScript, so adoption is incremental and reversible, and compute-heavy work can call into Zig or Rust through generated Wasm bindings without switching languages.
...
Teams can adopt VibeLang inside an existing TypeScript codebase and change
