...
func runWithFactory(args []string, stdout io.Writer, stderr io.Writer, pinned compilerFactory) int {
	...
	flags.StringVar(&forkCheckout, "fork-checkout", "", "exact smithersai/TypeScript checkout with a clean tsc tree")
	...
	if requestPath != "" {
		...
		request = compiler.CompileRequest{RootNames: flags.Args()}
	}
	...
	if err != nil && !errors.Is(err, compiler.ErrNotImplemented) {
		code := "VIBE_GO_BACKEND"
		message := err.Error()
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(ctx.Err(), context.DeadlineExceeded) {
			code = "VIBE_GO_TIMEOUT"
