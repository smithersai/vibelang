...
Frontend composition order is comptime first, then VibeLang lowering. Feed each
`loweredFiles[file].code` to the VibeLang project compiler under the same logical
file name. Compose the VibeLang map (VibeLang output to comptime-lowered input) as the
...
`vibelang-assets@4` for that record change. Hashing the recorded child key is
...
end-to-end `.vibe` lowering. The VibeLang emitter rewrites and strips attributes
...
(`VIBE1804`); and the root relative runtime graph rejects a VibeLang dynamic-import
...
diagnostics through the comptime map. Parsing, resolved-binding recognition,
bounded evaluation, schema reification, generated type aliases and replacement
text now come from the pinned Go TypeScript compiler. The JavaScript host only
supplies explicitly requested, root-confined tracked text, marshals cache
values and composes source maps from authored UTF16 ranges. There is no 5.9
compiler or evaluator fallback on this path. Generated TypeScript/`.vibe` replacements
...
- **Stack traces name no host path.** The loader module runs from a `data:` URL,
  ...
  keeps only `data:` frames, redacting the rest to `vibelang:loader-runner`.
