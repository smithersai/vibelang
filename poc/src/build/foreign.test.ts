  ...
  try {
    const makeTool = async (name: string, flavor: string): Promise<string> => {
      ...
      await writeFile(tool, `#!/usr/bin/env node
        ...
        if (!process.env.VIBELANG_TEST_MODE) { console.error("explicit environment missing"); process.exit(92) }
    ...
    const first = await new ForeignCompiler({
      cacheDirectory: join(root, ".cache"), zig: firstTool, environment: { VIBELANG_TEST_MODE: "one" },
    }).compile(source, "zig");
    const environmentChanged = await new ForeignCompiler({
      cacheDirectory: join(root, ".cache"), zig: firstTool, environment: { VIBELANG_TEST_MODE: "two" },
    }).compile(source, "zig");
    const toolChanged = await new ForeignCompiler({
      cacheDirectory: join(root, ".cache"), zig: secondTool, environment: { VIBELANG_TEST_MODE: "one" },
    }).compile(source, "zig");
...
test.skipIf(process.platform === "win32")("cache paths are canonical and ambient variables stay out of the key", async () => {
  ...
  try {
    ...
    const source = join(root, "main.zig");
    await writeFile(source, "export fn value() i32 { return 42; }\n");
    process.env.VIBELANG_AMBIENT_NOISE = "one";
    const linked = new ForeignCompiler({ cacheDirectory: join(root, "link", "cache") });
    process.env.VIBELANG_AMBIENT_NOISE = "two";
    ...
    delete process.env.VIBELANG_AMBIENT_NOISE;
    await rm(root, { recursive: true, force: true });
  }
}, 60_000);
