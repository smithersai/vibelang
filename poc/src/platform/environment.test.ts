...
describe("Environment", () => {
  test("ProcessEnvironment reads the live process environment", () => {
    ...
    try {
      ...
      expect(environment.get(name).unwrapOr("")).toBe("changed");
    } finally {
      delete process.env[name];
    }
    expect(ProcessEnvironment.make().get(name).isNone()).toBe(true);
  });
  ...
  test("MapEnvironment satisfies the contract from an in-memory map", () => {
    ...
    expect(environment.get("REGION").unwrapOr("")).toBe("eu-west-1");
    environment.unset("EXTRA");
    expect(environment.get("EXTRA").isNone()).toBe(true);
  });
  ...
  test("an empty value is present, and a prototype-shaped name is an ordinary name", () => {
    ...
    expect(environment.get("EMPTY").isSome()).toBe(true);
    expect(environment.get("EMPTY").unwrapOr("<absent>")).toBe("");
    expect(environment.get("__proto__").unwrapOr("<absent>")).toBe("not-a-prototype");
    expect(environment.names()).toEqual(["EMPTY", "__proto__"]);
    expect(environment.get("constructor").isNone()).toBe(true);
  ...
  test("Environment resolves through a Layer under its nominal key", () => {
    ...
    const region = (): string => Environment.context().get("REGION").unwrapOr("unknown");
