/**
 * @module
 * The trusted adapter between a VibeLang `Result` and the runtime's foreign
 ...
 * path, and `specification/failures.mdx` is explicit that authored VibeLang
 * cannot spell it: a reachable recoverable `Error` exit MUST widen the return
 * type into `Result<A, E>` (`VIBE1101`), and every alternative spelling is
 * refused too — `match({ ok, error: (e) => { throw e } })` is `VIBE1303`,
 * and aborting through a foreign `never`-returning function is `VIBE1301`
 * or `VIBE1101` depending on where the call sits. The one exit that keeps a
 ...
 * VibeLang." The *decision* about what is malformed stays in the `.vibe`, as an
 ...
 * TypeScript calling TypeScript, with no VibeLang row involved.
 *
 * @throws {never}
 */
...
/**
 * A `ValueCodec` whose `decode` throws the failure its VibeLang implementation
 * returned.
 *
