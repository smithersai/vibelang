 ...
 * - **A sparse array is refused.** A hole is an index below `length` that is
 *   *not* an own property, so it has no key to participate with, and freezing
 *   one would mint a slot that can never be read or written. `./array-shape.ts`
 *   holds the rule and the reasoning; conversion rejects here, and the array
 *   comparison and hash below compare index ownership before values so the two
 *   can never disagree about what a hole is.
 *
...
import { HOLE_HASH, requireDenseArray, sameArrayShape } from "./array-shape.ts";
...
function convert(value: unknown, pending: Set<object>): unknown {
  ...
  try {
    if (isArray) {
      // `Array.prototype.map` *preserves* a hole rather than converting it, so
      // gating first is what keeps the copy dense. See `./array-shape.ts`.
      const source = requireDenseArray(value as readonly unknown[], "a Data value");
      const items = source.map((item) => convert(item, pending));
...
function dataEquals(left: unknown, right: unknown, recurse: (left: unknown, right: unknown) => boolean): boolean {
  ...
  if (leftIsArray) {
    ...
    // Ownership before values, and it subsumes the length check: reading both
    // sides by index would report a hole and an own `undefined` as the same
    // member. Conversion already refuses a sparse array, so this cannot fire on
    // a value built here; it is what keeps the comparison honest if it ever can.
    if (!sameArrayShape(leftItems, rightItems)) return false;
...
function dataHash(value: unknown, recurse: (value: unknown) => number): number {
  ...
  if (Array.isArray(value)) {
    ...
    // `for…of` yields `undefined` at a hole, which would hash it as the member
    // it does not have. Ownership is folded in instead, matching `dataEquals`.
    for (let index = 0; index < value.length; index += 1) {
      accumulator = HashNamespace.combine(
        accumulator,
        Object.hasOwn(value, index) ? recurse(value[index]) : HOLE_HASH,
      );
    }
  } else {
