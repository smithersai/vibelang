...
import { requireDenseArray, sameArrayShape } from "./array-shape.ts";
...
function tuple<const Parts extends readonly EquivalenceValue<never>[]>(
  ...
  return make<readonly unknown[]>((left, right) => {
    ...
    // Index ownership before values: a hole and an own `undefined` both read as
    // `undefined`, and they are not the same member. See `./array-shape.ts`.
...
function array<T>(item: Equivalence<T>): Equivalence<readonly T[]> {
  ...
  return make<readonly T[]>((left, right) => {
    ...
    // Length *and* index ownership, before a single member is read.
    if (!sameArrayShape(left, right)) return false;
...
function checkLaws<T>(equivalence: Equivalence<T>, samples: readonly T[]): string | undefined {
  ...
  // Every loop below reads `samples[index]`, so a hole would be law-checked as
  // `undefined` and reported lawful over a sample that is not there.
  requireDenseArray(samples, "Equivalence.checkLaws samples");
