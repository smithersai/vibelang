...
export interface CompileProjectOptions extends AnalyzeProjectOptions {
   ...
   * stage. They participate only in relative-import rewriting; Vibe never
...
export function compileProject(
  ...
  if (options.preserveVibeSpecifiers !== undefined && typeof options.preserveVibeSpecifiers !== "boolean") {
    throw new TypeError("compileProject preserveVibeSpecifiers must be a boolean");
  }

  const outputBySource = new Map<string, string>();
  const outputOwners = new Map<string, string>();
  const stripImportAttributesForSources = new Set<string>();
  const vibeSourceNames = new Set<string>();
  for (const source of sources) {
    const absolute = resolve(semantic.rootDir, source.fileName);
    vibeSourceNames.add(absolute);
  ...
  for (const source of [...sources].sort((left, right) => compareText(left.fileName, right.fileName))) {
    ...
    const compiled = compileSemanticModel(source.source, {
      ...
      sourceName,
      preserveVibeSpecifiers: options.preserveVibeSpecifiers,
    }, model, { outputBySource, stripImportAttributesForSources, vibeSourceNames });
