...
/**
 * `VIBE1802` is the fail-closed backstop for a cross-module function that
...
function project(consumer: string, seam = CAPABILITY_SEAM, rootDir = "/virtual/accessor-rows") {
  return analyzeProject(
    [{ fileName: "seam.vibe", source: seam }, { fileName: "main.vibe", source: consumer }],
    { rootDir },
  );
}
...
describe("an accessor access is an ordinary call", () => {
  test("every spelling of an accessor access charges the accessor's requirement row", () => {
    ...
    // get-only cross-module READ was refused, and only by VIBE1802.
    ...
    for (const [label, body] of spellings) {
      const analysis = project(`
        import { Stamp } from "./seam.vibe"
        export function read(source: Stamp): number { ${body} }
      `);
      expect({ [label]: codes(analysis) }).toEqual({ [label]: [] });
      expect({ [label]: analysis.files["main.vibe"].rows.read.requirements })
        .toEqual({ [label]: ["Clock"] });
    }
  });
  ...
  test("a method call still charges its row, and an infallible accessor charges nothing", () => {
    ...
    expect(method.files["main.vibe"].rows.read.requirements).toEqual(["Clock"]);

    const plain = project(
      `
        import { Box } from "./seam.vibe"
    ...
    expect(plain.files["main.vibe"].rows.read.requirements).toEqual([]);
  });
  ...
  test("a top-level accessor access reports its unsatisfied requirements", () => {
    ...
    // top-level call — which has always reported VIBE2102.
    for (const body of [
      ...
      `);
      expect({ [body]: codes(analysis) }).toEqual({ [body]: ["VIBE2102"] });
    }
  });
  ...
  test("a provide site sees a requirement that only an accessor read introduces", () => {
    ...
    const analysis = project(`
      ...
      import { Stamp, stamp } from "./seam.vibe"
    ...
    `);
    expect(codes(analysis)).toEqual(["VIBE2101"]);
    expect(analysis.diagnostics.find((diagnostic) => diagnostic.code === "VIBE2101")?.message)
      .toContain("Clock");
  });
});

describe("VIBE1802 refuses values, not calls", () => {
  test("an ordinary cross-module call is accepted wherever its row is attributed", () => {
    ...
    for (const [label, body] of accepted) {
      const analysis = project(
        `import { register, count } from "./seam.vibe"\n${body}\n`,
        REGISTRY_SEAM,
      );
      expect({ [label]: codes(analysis) }).toEqual({ [label]: [] });
    }
  ...
  test("a cross-module function that becomes a value is still refused", () => {
    ...
    for (const [label, body] of refused) {
      ...
      expect({ [label]: codes(analysis) }).toEqual({ [label]: ["VIBE1802"] });
    }
  });
