...
test("same-named Errors in different modules keep distinct runtime identities", async () => {
  ...
  const root = await mkdtemp(join(tmpdir(), "vibelang-qualified-rows-"));
  try {
    ...
    expect(compiled.files["left.vibe"]!.code).toContain('"vibelang:left.vibe:Duplicate"');
    expect(compiled.files["nested/right.vibe"]!.code).toContain('"vibelang:nested/right.vibe:Duplicate"');
    ...
    expect(errorIdentity(left)).toBe("vibelang:left.vibe:Duplicate");
    expect(errorIdentity(right)).toBe("vibelang:nested/right.vibe:Duplicate");
