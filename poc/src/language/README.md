...
Public source/project analysis and compilation, generated-code checking,
declaration emission and declaration metadata text operations use the pinned
native Go compiler.
They return serializable facts, not a 5.9 `SourceFile` or `Diagnostic`.
The executable-body, Plan/Manifest and schema frontends also use native Go
checking, lowering and codec derivation. The aggregate language entry is Go-only;
there is no TypeScript 5.9 compiler-library dependency or fallback. TypeScript
hosts transport serializable requests and assemble artifacts rather than parse
or type-check source themselves. The Go-only migration and its release evidence
are recorded in [the migration record](../../../compiler/GO-MIGRATION.md).

The separate durable-model migration is unfinished. Historical body/Manifest
compatibility APIs are retained, also Go-backed; their existence does not prove
the new keyed Plan default or its packaged scheduler host is complete. See the
[Plan interoperability worklist](../../../compiler/SMITHERS-INTEROP.md). <!-- brand-gate: allow external library name -->

## Public API

The programmatic compiler APIs are Node-compatible and return artifacts in
memory, without writing user output files. Native compiler preparation in a
development checkout may populate its compiler cache:
...
if (!checked.ok) {
  // result.analysis.diagnostics include native language and source TS checks.
  // emitDiagnostics report the separate native check of generated SDK code.
}
```

- `analyzeSource(source, { fileName?, rootDir? })` uses the pinned native Go
  compiler and returns discovered errors, function channels, inferred
  failure/requirement rows, and source-located diagnostics. Optional `rootDir`
  bounds dependency discovery; it defaults to cwd for relative source names or
  the containing directory for absolute names. `parseErrors` and `parseFunctions`
  use the same query. Refused-source rows are provisional, not checked proofs.
- `analyzeProject([{ fileName, source }], { rootDir?, additionalRuntimeSources? })`
  uses one native Go query for the explicitly supplied `.vibe` modules. It resolves relative
  ...
  diagnostics. Foreign dependencies may resolve beneath the root; omitted
  `.vibe` files stay excluded. Runtime aliases preserve target symbol identity;
  nominally issued data modules must additionally pass Go's closed-data validation.
  Use an explicit root for portable multi-file nominal names. It performs
  analysis only and does not emit files or manufacture a checked deployment proof.
- `compileVibeLang(source, options)` returns generated TypeScript, the analysis,
  and a deterministic version-3 source map with embedded authored content.
  Native source checking must succeed before code is exposed; use the checked
  API to additionally validate it against the selected SDK and dependencies.
- `compileProject(sources, options)` lowers the complete supplied in-memory
  ...
  returns virtual files without writing them. `preserveVibeLangSpecifiers: true`
...
- `compileAndCheckVibeLang(source, options)` is the acceptance API: `ok` is true
...
- `emitProjectDeclarations([{ fileName, code, effects?, runtimeModule? }])`
  ...
  `@vibelangEffects` metadata; `readDeclarationEffects(code, fileName?)` is the
...
- `VIBE6001`: a mandatory compiler option is missing, or is stated with a
  value other than `true`.
- `VIBE6002`: a forbidden compiler option appears in the configuration.
...
- `VIBE6003`: an option nothing classifies. The fail-closed reading of the
...
program, because §Mandatory governs the legality of authored `.vibe` and not the
shape of this compiler's output.

The bounded project/ownership checks use stable codes:

- `VIBE1002` is historical: it guarded the retired JavaScript frontend's private
  parser-diagnostics field. That frontend has been removed; native analysis
  obtains syntax diagnostics from the pinned Go parser.
- `VIBE1107`: class `static {}` initialization blocks are unsupported; they
  execute outside every checked function channel.
- `VIBE1205`: a `panic(...)` or postfix `!`
...
- `VIBE1601`/`1602`/`1603`: the host-global prohibition, and it is an
  **allowlist**, not a list of forbidden spellings. Authored `.vibe` may reach the
  ...
  `types: []` for the authored-`.vibe` program and the pinned fork carries no
...
- `VIBE1604`: **dynamic code evaluation.** `eval` and `Function` are in
  ...
  instant (the direct spelling is `VIBE1602`), `eval("Math.random()")` →
  randomness (`VIBE1603` directly), `new Function("return process.platform")()`
...
- `VIBE1605`: **determinism-hostile globals.** `WeakRef`,
  ...
  row, in the same file where the `Date.now()` control reported `VIBE1602`.
  Its own code rather than `VIBE1601` because 1601's message ends "access it
  ...
  it". `VIBE1604` is the precedent — own code, own reason, for the same
  "there is no capability that could provide this" argument. Unlike
  `VIBE1604` the line is the NAME, not the operation, because every value use
  ...
  discriminator that picks `VIBE1602`/`1603`/`1601` — no ambient site
  ...
  is why this file's `Clock`/`Random` walls stay `VIBE1602`/`1603` and
...
- `VIBE1802`: a cross-module function becomes a **value** that escapes
...
- `VIBE1704`, `VIBE1706` and `VIBE1708` are retired, and the reason
  ...
  "VibeLang MUST NOT add: `if` or `switch` in expression position, braced or
  ...
  - `VIBE1703` and `VIBE1717` are the only surviving members of the
...
- `VIBE1303`: an inferred-fallible function crosses a general callback
  ...
  still computes `!` with the non-null-assertion meaning `.vibe` abolishes and a
  propagating callback would otherwise appear to return the Result it extracts
  from. `VIBE1404` below is deliberately *not* widened the same way: it is a
  rule about started work, not about a lost failure channel.
- `VIBE1404`: an async callback has no compiler-recognized owner. The rule is
  over *started* work, so the recognized owners are `Result.tryPromise`, a
  consumed `Layer.provide`, and the `vibelang:native` pin — `native(fn)` receives
...
- `VIBE2106`: the receiver of `Capability.context()` does not identify
...
- `VIBE2107`: a **detached** reference to the compiler-recognized
  ...
  is why `vibe inspect` reported `ok: true, requirements: []` for a file
  `vibe check` refused, and why the Go fork's looser prelude typing accepted
...
- **A requirement crosses a callback boundary.** The boundary was already
  ...
  (`VIBE1303`) and an async one is refused (`VIBE1404`) — but the R row
  ...
  encoding open).
- `VIBE2102` is charged at module scope for a capability read written
...
- `VIBE1504`: a **call-like** foreign form whose Result lowering is deferred
  ...
  **tagged template** (the tag is a call that never appears in call position),
  ...
  `VIBE1505`: a checked foreign panic channel appears at top level.
...
- `VIBE1502` also refuses two `@throws` claims that cannot be honoured, both
...
- `VIBE1508`: executable foreign provenance escapes through an unsupported
  ...
  **untrusted** foreign code outside the checked call scope.
  ...
  `VIBE1303`/`VIBE1802`), `isAmbientGlobalReference` (for
  `VIBE1601`/`1602`/`1603`), and the provenance walks all route through it.
  Four rules have had this defect: `VIBE1303` and `VIBE1802` were closed
  ...
  `VIBE1508` and `VIBE1601` were still open afterwards precisely because
  ...
  **"Which member does this key select?"** is `staticKeyName`, and the criterion
  ...
  `a-computed-context-access-charges-the-same-row` certifies as `VIBE2102`.
  ...
  **"Where does this value end up?"** is `forwardingParent`, the PARENT direction
  ...
  reported no `VIBE1104` at all, while the byte-adjacent `as` spelling
  ...
  A **trusted** call therefore falls through to the `VIBE1508` argument
  ...
  callback's own inferred Result channel stays `VIBE1303`, started async work
  stays `VIBE1404`, and a host global in the callback body stays
  `VIBE1602`; none of those consults the boundary's trust. Both directions
  are gated in `foreign-callback-trust.test.ts` and, on the fork, in
  `compiler/fork_callback_trust_test.go`.
- `VIBE1510`: a TypeScript/JavaScript module edge lacks a leading JSDoc
...
- `VIBE1201`: a compiler hook is being used as an author-facing Result
  ...
  part of the ordinary VibeLang authoring API." The first spelling is
  ...
  under a name a VibeLang author could reach". Measured before the rule:
  `import { __vsResultSuccess } from "vibelang/context"` compiled with zero
...
- A generic function whose declared `Result` error mentions its own type
  ...
  `@vibelangEffects` declaration metadata. Because two authored
...
- **An accessor access is an ordinary call.** Reading `box.size` CALLS the
  ...
  — and a top-level accessor access reports `VIBE2102` exactly as a
  ...
  `VIBE1802`, incidentally. A same-module accessor, a setter, and a get/set
...
- **An authored `panic(...)` exit does not widen a return type.** It is not
  ...
  cannot draw `VIBE1101`/`1102`/`1105`/`1106` on its own. `specification/
  ...
  the placement rule (`VIBE1503`) and the lowering read it.
  ...
  - Everything that is not a panic is untouched. A **recoverable** `throw` still
    obeys the older MUST on the same page — "A `.vibe` function with a reachable
    recoverable Error exit MUST return or infer a Result" — so `VIBE1101`,
...
- Property/accessor evaluation, constructors, and higher-order escapes use a
  ...
  into foreign code is rejected until a VibeLang-owned Result/task adapter defines
...
- Produced Results and Promises must be consumed. Promise instance chaining is
  ...
  now charges its CALLER (`VIBE1301`, or `VIBE1402` for a container of
...
- Row members are module-qualified nominal identities. A name unique across the
  ...
  rows, `Layer.succeed` providers, `@vibelangEffects` declaration metadata, and
...
- Module trust is exact: only the declared compiler modules
  `vibelang/context`, `vibelang/provider`, `vibelang:exceptions`,
  `vibelang:comptime`, and `vibelang:flows` are compiler intrinsics. Prefixes
...
diagnostics (`VIBE1001`); a retired form the sweep does not claim survives
as the grammar rule `VIBE1000`. Recognition is a **grammar** property, not
...
`VIBE1000` also owns the switch clause separator. Switch clauses are
...
- Project analysis, lowering, stock checking, and declaration emit are no-write
  ...
  on each call. Declaration `@vibelangEffects` metadata is a strict versioned POC
...
- Must-consume checking uses native checker identities and path-sensitive
  control-flow analysis. Its value/provenance traversal remains bounded and
  conservative rather than an unrestricted heap or lifetime proof.
- Postfix `!` uses expression-position delimiters, preserving short-circuiting,
  loop headers and evaluation order. Non-Result operands, forbidden enclosing
  members, catch boundaries and effectful authored generators remain refused.
  Ordinary SDK function calls remain eager, including in async functions,
  methods and host-invoked callbacks; durable resumability is a private ABI.
...
- Source maps are high-resolution but deliberately incomplete: a transformed
  ...
  the intermediate file is uniquely identifiable. Public compilation obtains
  node ranges and maps from the pinned native AST emitter, as does the private
  executable-body profile. Runtime stack-mapping
  tests cover the supported public transformations; richer debugger scope
  metadata remains outside the current map format.

These remaining gaps stay diagnostics or explicit integration constraints;
neither provisional analysis nor intermediate code is deployment authority.
