...
`)
  ...
  // A raw `for` around `sleep(1)` used to be `VIBE4107`. `MIGRATION-PLAN.md`
...
`
  ...
  const loop = compileDurableFlow(loopSource, { fileName: "flows/pause.vibe.ts", flowId: "test/timer/Pause" })
...
test("restart after scheduling reuses the exact persisted deadline", async () => {
  const directory = mkdtempSync(join(tmpdir(), "vibelang-timer-restart-"))
