...
> directory", `grep -rl SMITHERS300 conformance/corpus/` returns 0 files, and no
...
one ships publicly as `smthrs/targets` (**true when written; both are gone now** —
the portability targets were withdrawn on 2026-08-23 and the `./targets` subpath
was removed from the export map, so `import("smthrs/targets")` today fails with
