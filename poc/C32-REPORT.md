...
The Go fork now implements the checked `vibelang:native` intrinsic. All five cases in
...
The registered authored specifier is exactly `vibelang:native`. It resolves during checking to a
dedicated compiler-owned declaration file, `__vibelang_native.d.ts`; recognition follows the
resolved function declaration back to that file. It never tests the use-site spelling `native`,
and the compiler-module registry uses exact resolved-module identities rather than a `vibelang:`
prefix.
...
| Case | Evidence |
| --- | --- |
| renamed import | `import { native as pin } ...; pin(checksum)` emits `VIBE3001` |
| namespace read | `import * as compiler ...; compiler.native(checksum)` emits `VIBE3001` |
...
`VIBE3005`; retired `/** @native */` markers warn with `VIBE3006` and do not pin.

The transitive conformance case emits exactly:

```text
native pin failed: TypeScript is required through a-pin-reaching-typescript-transitively-is-rejected.vibe#checksum -> a-pin-reaching-typescript-transitively-is-rejected.vibe#first -> a-pin-reaching-typescript-transitively-is-rejected.vibe#second -> a-pin-reaching-typescript-transitively-is-rejected.vibe#third -> a-pin-reaching-typescript-transitively-is-rejected.vibe#boundary
```

The host-module case resolves `node:fs` through a checker-only declaration shim and therefore lets
the existing host/foreign rules own the result: `VIBE1510` remains at the foreign import use,
and the pin emits `VIBE3001` with `Module<"node:fs">` and the `checksum -> hostBacked` path.
...
- `go test ./compiler ./cmd/vibec-go -count=1` — pass (`compiler` 210.083s, CLI 8.598s)
