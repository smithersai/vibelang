...
// ---------------------------------------------------------------------------
// Jumps escaping a labeled value construct — SMITHERS1714 / SMITHERS1715
// ---------------------------------------------------------------------------

// TestPinnedForkValueBreakMayNotEscapeItsConstruct pins the two shapes the
// specification has NOT finalized (control-flow.mdx, "Loop Else", Direction:
// "The exact treatment of infinite loops, unreachable completion, and nested
// label selection is not finalized") together with the four shapes it has.
func TestPinnedForkValueBreakMayNotEscapeItsConstruct(t *testing.T) {
	runFailClosedCases(t, []failClosedCase{
		{
			name: "a cross-construct value break is rejected",
			source: "export function firstPair(rows: number[][]): string {\n" +
				"  const found = outer: for (const row of rows) {\n" +
				"    const inner = search: for (const value of row) {\n" +
				"      if (value > 10) break :outer \"big\"\n" +
				"      break :search `${value}`\n" +
				"    } else \"empty row\"\n" +
				"    if (inner === \"empty row\") break :outer inner\n" +
				"  } else \"nothing\"\n" +
				"  return found\n" +
				"}\n" +
				"\n" +
				"export function main(): string[] {\n" +
				"  return [firstPair([[1], [20]])]\n" +
				"}\n",
			reject: []string{"SMITHERS1715@4:40"},
		},
		{
			name: "a plain `break label` out of a value block is rejected",
			source: "export function classify(input: string): string {\n" +
				"  const kind = verdict: {\n" +
				"    if (input.length === 0) break verdict\n" +
				"    break :verdict \"short\"\n" +
				"  }\n" +
				"  return kind\n" +
				"}\n" +
				"\n" +
				"export function main(): string[] {\n" +
				"  return [classify(\"\"), classify(\"ada\")]\n" +
				"}\n",
			reject: []string{"SMITHERS1714@3:29"},
		},
		{
			name: "a plain `break label` out of a value LOOP flows into its else",
			source: "export function firstPassing(scores: number[]): number {\n" +
				...
				"\n" +
				"export function main(): string[] {\n" +
				"  return [`${firstPassing([12, -1, 78])}`, `${firstPassing([12, 78])}`]\n" +
				"}\n",
			stdout: "-1\n78",
		},
		{
			name: "an unlabeled break inside a nested loop stays contained",
			source: "export function firstEven(rows: number[][]): number {\n" +
				"  const found = search: for (const row of rows) {\n" +
				...
				"\n" +
				"export function main(): string[] {\n" +
				"  return [`${firstEven([[1, 3], [4]])}`, `${firstEven([[-1]])}`]\n" +
				"}\n",
			stdout: "4\n-1",
		},
		{
			name: "a value break naming a label nested INSIDE the construct is contained",
			source: "export function firstPair(rows: number[][]): string {\n" +
				"  const found = outer: for (const row of rows) {\n" +
				"    const inner = search: for (const value of row) {\n" +
				"      break :search `${value}`\n" +
				...
				"\n" +
				"export function main(): string[] {\n" +
				"  return [firstPair([[7], [20]]), firstPair([])]\n" +
				"}\n",
			stdout: "7\nnothing",
		},
		{
			name: "a value block whose every exit carries a value is accepted",
			source: "export function classify(input: string): string {\n" +
				"  const kind = verdict: {\n" +
...
// ---------------------------------------------------------------------------
// Type-only imports — the fail-CLOSED direction of the same locked rule
// ---------------------------------------------------------------------------

// TestPinnedForkTypeOnlyImportAddsNoRuntimeRequirement is the safe-direction
// companion: specification/compatibility.mdx, "Runtime TypeScript Dependency"
// (Locked) — "Using a value from TypeScript or JavaScript code MUST add a
// runtime requirement representing that compatibility boundary. A type-only
// import MUST NOT add that requirement." The pair of cases below imports the
// SAME untrusted module twice, once for a type and once for a value, and the
// difference in verdict is the whole obligation.
