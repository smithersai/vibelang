...
// where the consequence is larger: the marker suppresses VIBE1510, whose
...
// VIBE1510 at the import specifier rather than compiling with a quietly
...
func TestPinnedForkThrowsNeverIsCaseSensitive(t *testing.T) {
	const source = "import { Panic } from \"vibelang:exceptions\"\n" +
...
`
	runFailClosedCases(t, []failClosedCase{{
		...
		source: "import { Upper } from \"./foreign.ts\"\n" +
			...
			"  return [\"ok\"]\n" +
			"}\n",
		reject: []string{"VIBE1101@2:1", "VIBE1504@3:3"},
	}})
}
